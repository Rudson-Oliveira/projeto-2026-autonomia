...
class UiPathService
{
    ...
    protected $tenantId; // Adicionado
    protected $refreshToken; // Adicionado
    ...
    public function __construct()
    {
        ...
        $this->tenantId = env('UIPATH_TENANT_ID'); // Adicionado
        $this->refreshToken = env('UIPATH_REFRESH_TOKEN'); // Adicionado
    }
    ...
    public function startProcess(string $processName, array $inputArguments = []): ?array
    {
        ...
        try {
            $response = Http::withToken($token)
                ->post("{$this->orchestratorUrl}/odata/{$this->tenantId}/Jobs/UiPath.Server.Configuration.OData.StartJobs", [
                    'startInfo' => [
    ...
    public function getJobStatus(int $jobId): ?array
    {
        ...
        try {
            $response = Http::withToken($token)
                ->get("{$this->orchestratorUrl}/odata/{$this->tenantId}/Jobs(" . $jobId . ")");
